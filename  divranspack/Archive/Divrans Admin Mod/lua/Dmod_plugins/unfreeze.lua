...
-------------------------------------------------------------------------------------------------------------------------
local DmodPlugin = {}
DmodPlugin.ChatCommand = "unfreeze" -- The chat command you need to use this plugin
DmodPlugin.Name = "Unfreeze" -- The name of the plugin
DmodPlugin.Description = "Unfreeze someone." -- The description shown in the Menu
...
if (Dmod_CheckRequiredRank(ply, DmodPlugin.RequiredRank)) then
	if (Args[2]) then
		if (Dmod_FindPlayer(Args[2])) then
			...
			T:UnLock()
			Dmod_Message(true, ply, ply:Nick() .. " unfroze " .. T:Nick() .. ".","punish")
		else

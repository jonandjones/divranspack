...
-- The sprite material is fucked up.
--tMats.Glow1 = Material("sprites/light_glow01")
...
for _, material_v in pairs(tMats) do
	material_v:SetInt("$spriterendermode",9)
	material_v:SetInt("$ignorez",1)
	material_v:SetInt("$illumfactor",8)
end
...
local startpos = self.Position
	...
	render.DrawSprite(startpos, 200*self.GSize,40*self.GSize,Color(80, math.random(80,90), math.random(230,255),self.GAlpha))
	render.DrawSprite(startpos, 30*self.GSize,140*self.GSize,Color(80, math.random(80,90), math.random(240,255),0.7*self.GAlpha))
	--shockwave
	-- This is a translucent ring that originates from the blast.
